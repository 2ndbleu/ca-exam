...
def show_usage(name):
    ...
    print("Usage: %s [-l n] [-c m] [-t x] filename" % name)
    ...
    print("\t-t shows the pipeline diagram (default: 0)")
    print("\t   0: shows no output message")
    print("\t   1: shows the diagram for retired instructions at the end of the execution")
    print("\t   2: shows the diagram for fetched instructions at the end of the execution")


def parse_args(args):
    if (not len(args) in [ 2, 4, 6, 8, 10 ]):
        return None
        ...
        if args[index].startswith('-'):
            if args[index] == '-l':
                ...
                try:
                    ...
                    print("Invalid option '%s'" % args[index + 1])
                    return None
                index += 2
                Log.trace = True if x > 0 else False
                Log.trace_bubble = True if x >= 2 else False
            elif args[index] == '-o':
                try:
                    out_flags = int(args[index + 1])
                except ValueError:
                    print("Invalid option '%s'" % args[index + 1])
                    return None
                index += 2
                Log.trace_flags = out_flags
            else:
...
def main():
    ...
    outfile_name = None
    if Log.trace_flags > 0:
        now = datetime.now()
        infile_name = filename.replace('.', '+').replace('/', '__').replace('\\', '__')
        outfile_name = infile_name + f"--{now.strftime('%Y-%m%d-%H%M%S_%Z')}"
    if Log.trace:
        print()
    Pipe.trace.dump(Log.trace, Log.trace_bubble, outfile_name)
